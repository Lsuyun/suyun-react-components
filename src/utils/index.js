 ...
 * @LastEditTime: 2022-01-26 21:25:51
 * @Description:
 */
// 数组去重
export const arrNotequal = (arr, key) => {
  const obj = {};
 ...
 * @return {*}  返回全部的父级 []
 */
export const getAllParentN = (arr, id, key, parentId = -1) => {
  let res = [];
  arr.forEach((item) => {
    if (item[key] == id) {
      res.push(item);
 ...
 * @return {*}
 */
export const getTreeData = (arr, key = 'id', parentId = -1) => {
  arr.forEach((item) => {
...
// 扁平化数组
export const getFlatArr = (treeArr) => {
  let res = [];
  treeArr.map((item) => {
    let data = {};
...
// export { arrNotequal, getAllParentN, getTreeData, getFlatArr };
